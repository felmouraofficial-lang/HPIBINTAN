...
export function SiteHeader() {
  ...
  return (
    <header className="fixed left-0 right-0 top-3 z-50 text-white transition duration-500 md:top-4">
      <div className={`container flex h-[58px] items-center justify-between gap-3 rounded-[1.75rem] px-4 transition duration-500 md:h-[64px] md:rounded-[2rem] ${solid ? "bg-white/86 text-zinc-950 shadow-[0_18px_70px_rgba(15,23,42,.14)] ring-1 ring-black/5 backdrop-blur-2xl dark:bg-zinc-950/82 dark:text-white dark:ring-white/10" : "bg-white/14 shadow-[0_18px_70px_rgba(0,0,0,.14)] ring-1 ring-white/18 backdrop-blur-2xl"}`}>
        <Link href="/" className="flex min-w-0 shrink-0 items-center gap-2 xl:gap-3" onClick={() => setOpen(false)}>
          <span className="grid h-11 w-20 shrink-0 place-items-center transition xl:h-[52px] xl:w-24">
            <Image src={logos[0]} alt="Logo DPC HPI Kabupaten Bintan" width={100} height={54} className="h-auto max-h-11 w-full object-contain drop-shadow-[0_10px_24px_rgba(0,0,0,.26)] xl:max-h-[52px]" priority />
          </span>
          <span className="hidden min-w-0 sm:block">
            <span className="block whitespace-nowrap text-xs font-black leading-tight tracking-wide 2xl:text-sm">DPC HPI Kabupaten Bintan</span>
        ...
        <nav className={`hidden items-center gap-1 rounded-full p-1 text-[12px] font-black xl:flex 2xl:text-[13px] ${solid ? "bg-zinc-100/80 dark:bg-white/8" : "bg-white/10"}`}>
          {nav.map(([label, href]) => (
        ...
        <div className="hidden items-center gap-2 xl:flex 2xl:gap-3">
          ...
          <Link href={guideBookingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#ff9900] px-4 py-2.5 text-xs font-black uppercase tracking-[.1em] text-white shadow-[0_18px_45px_rgba(255,153,0,.28)] transition duration-300 hover:-translate-y-0.5 hover:bg-primary 2xl:px-6 2xl:py-3 2xl:tracking-[.12em]">
            <CalendarCheck className="h-4 w-4" />Guide Booking
          </Link>
        </div>

        <button type="button" onClick={() => setOpen((value) => !value)} className={`grid h-11 w-11 place-items-center rounded-full border xl:hidden ${solid ? "border-zinc-200 bg-white text-zinc-900" : "border-white/20 bg-white/10 text-white"}`} aria-label="Menu">
