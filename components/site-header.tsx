    ...
    <header className="fixed left-0 right-0 top-3 z-50 text-white transition duration-500 md:top-4">
      <div className={`container flex h-[60px] items-center justify-between gap-3 rounded-[1.75rem] px-4 transition duration-500 md:h-[68px] md:rounded-[2rem] ${solid ? "bg-white/86 text-zinc-950 shadow-[0_18px_70px_rgba(15,23,42,.14)] ring-1 ring-black/5 backdrop-blur-2xl dark:bg-zinc-950/82 dark:text-white dark:ring-white/10" : "bg-white/14 shadow-[0_18px_70px_rgba(0,0,0,.14)] ring-1 ring-white/18 backdrop-blur-2xl"}`}>
        <Link href="/" className="flex min-w-0 shrink-0 items-center gap-2 xl:gap-3" onClick={() => setOpen(false)}>
          <span className="grid h-11 w-20 shrink-0 place-items-center transition xl:h-14 xl:w-24">
            <Image src={logos[0]} alt="Logo DPC HPI Kabupaten Bintan" width={100} height={54} className="h-auto max-h-11 w-full object-contain drop-shadow-[0_10px_24px_rgba(0,0,0,.26)] xl:max-h-14" priority />
          </span>
          <span className="hidden min-w-0 sm:block xl:max-w-[260px]">
            <span className="block whitespace-nowrap text-xs font-black leading-tight tracking-wide xl:text-sm 2xl:text-base">DPC HPI Kabupaten Bintan</span>
        ...
        <div className="hidden items-center gap-2 xl:flex 2xl:gap-3">
          ...
          <Link href={guideBookingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#ff9900] px-4 py-3 text-xs font-black uppercase tracking-[.1em] text-white shadow-[0_18px_45px_rgba(255,153,0,.28)] transition duration-300 hover:-translate-y-0.5 hover:bg-primary 2xl:px-6 2xl:tracking-[.12em]">
