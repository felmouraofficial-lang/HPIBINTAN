...
import { fallbackMeetings, fallbackMembers } from "@/lib/fallback-data";
import { formatDate } from "@/lib/utils";

export default async function MeetingAttendancePage() {
  let members = fallbackMembers.map(({ id, name, position }) => ({ id, name, position }));
  let meetings = fallbackMeetings;
  try {
    const [dbMembers, dbMeetings] = await Promise.all([prisma.member.findMany({ where: { isActive: true }, orderBy: { name: "asc" }, select: { id: true, name: true, position: true } }), prisma.meeting.findMany({ orderBy: { date: "asc" }, take: 12, select: { id: true, title: true, date: true, time: true, location: true, description: true } })]);
    members = dbMembers.length ? dbMembers : members;
    meetings = dbMeetings.length ? dbMeetings.map((meeting) => ({ ...meeting, description: meeting.description ?? "" })) : meetings;
  } catch {}
