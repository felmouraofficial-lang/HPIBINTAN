...
export default function MeetingDocumentationPage() {
        ...
        <section className="container pb-16">
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {photos.map((photo, index) => (
              <button key={photo.src} type="button" onClick={() => setActive(index)} className={`group overflow-hidden rounded-[1.75rem] bg-white p-3 text-left shadow-soft transition duration-300 hover:-translate-y-1 hover:shadow-[0_28px_80px_rgba(47,33,25,.16)] dark:bg-white/8 ${index % 5 === 0 ? "lg:row-span-2" : ""}`}>
