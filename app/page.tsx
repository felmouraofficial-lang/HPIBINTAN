...
async function getHomeData() {
  const [home, websiteSettings, profile, members, announcements, agendas, meetings, gallery, transport, contact, destinations, legacySettings] = await Promise.all([
    safe(() => prisma.homeContent.findFirst(), null),
    safe(() => prisma.websiteSetting.findFirst(), null),
    safe(() => prisma.organizationProfile.findFirst(), null),
    safe(() => prisma.member.findMany({ where: { isActive: true }, orderBy: { createdAt: "asc" }, take: 4 }), []),
    safe(() => prisma.announcement.findMany({ where: { isPublished: true }, orderBy: { publishedAt: "desc" }, take: 3 }), []),
    safe(() => prisma.agenda.findMany({ where: { isPublished: true }, orderBy: { date: "asc" }, take: 3 }), []),
    safe(() => prisma.meeting.findMany({ orderBy: { date: "asc" }, take: 3 }), []),
    safe(() => prisma.gallery.findMany({ where: { isPublished: true }, orderBy: [{ sortOrder: "asc" }, { createdAt: "desc" }], take: 6 }), []),
    safe(() => prisma.transportation.findMany({ where: { isPublished: true }, orderBy: { createdAt: "desc" }, take: 4 }), []),
    safe(() => prisma.contact.findFirst(), null),
    safe(() => prisma.destination.findMany({ where: { isPublished: true }, orderBy: [{ featured: "desc" }, { createdAt: "desc" }], take: 12 }), []),
  ...
  const homeContent = home ?? {
    ...
    heroCtaText: "Hubungi Sekretariat",
    heroCtaUrl: "https://wa.me/6281275657026?text=Halo%20HPI%20Bintan%2C%20saya%20ingin%20booking%20guide.",
    heroImage: setting.hero_image || fallbackSettings.hero_image,
    aboutTitle: "Tentang Organisasi",
    aboutContent: profile?.history || fallbackProfile.history,
    visionTitle: "Visi",
    visionContent: profile?.vision || fallbackProfile.vision,
    missionTitle: "Misi",
    missionContent: profile?.mission || fallbackProfile.mission,
  };
  return {
    home: homeContent,
    settings: websiteSettings,
    legacySetting: { ...fallbackSettings, ...setting },
    ...
    agendas: agendas.length ? agendas : meetings.length ? meetings.map((item) => ({ id: item.id, title: item.title, date: item.date, time: item.time, location: item.location, badge: "Kegiatan", shortDescription: item.description || `${item.time} - agenda kegiatan HPI Pulau Bintan.`, coverImage: null })) : fallbackMeetings.map((item) => ({ id: item.id, title: item.title, date: item.date, time: item.time, location: item.location, badge: "Kegiatan", shortDescription: item.description || "Agenda kegiatan HPI Pulau Bintan.", coverImage: null })),
...
export default async function HomePage() {
  const { home, legacySetting, members, announcements, agendas, gallery, transport, contact, destinations, partners } = await getHomeData();
  const heroImage = home?.heroImage || legacySetting.hero_image || "/hero-bintan.jpg";
